...
class CreateNewCompany{
    createcompany()
    {
        ...
        userData.forEach(user=>{// retriveing testdata from eachrow in excel.
            test(`Create new company contact for`, async t => { //declare test
                ...
                await mouseclick(ContactLoc.NewCompany)
                await textbox(ContactLoc.Companyname,user.firstname)
               // await textbox(ContactLoc.LastName,user.lastname)
                ...
                await t.expect((ContactLoc.NameCreated).innerText).eql(user.firstname)
                await t.takeScreenshot(`Contact Created ${user.firstname}.png`)//Screenshot
...
export default new CreateNewCompany()
